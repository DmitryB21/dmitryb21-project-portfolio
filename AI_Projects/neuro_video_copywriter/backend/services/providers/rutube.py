"""
@file: backend/services/providers/rutube.py
@description: Адаптер загрузки медиаконтента из RuTube.
...
class RuTubeAdapter(BaseProviderAdapter):
    """Адаптер загрузки видео с RuTube."""
    ...
    def download(self, url: str, destination: Path) -> DownloadResult:
        ...
        logger.info("Starting RuTube download", extra={"url": sanitized_url})

        temp_dir = Path(tempfile.mkdtemp(prefix="rutube-yt-dlp-", dir=str(destination)))
        ...
        try:
            ...
            if not downloaded_files:
                raise ProviderError("yt-dlp did not produce any files for RuTube source")
    ...
    def _fetch_metadata(self, url: str) -> dict:
        ...
        try:
            ...
            logger.warning("Failed to fetch metadata via yt-dlp for RuTube", extra={"url": url})
            raise ProviderError("Cannot fetch metadata for RuTube source") from exc
